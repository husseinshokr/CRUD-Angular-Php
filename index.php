<html>
    ...
    <body>
        ...
        ng-init="fetchData()">
            <br />
            <h3 align="center">Inline Table Add Edit Delete Using 
                        AngularJS in PHP Mysql</h3><br />
            <div class="table-responsive">
				<div class="alert alert-success alert-dismissible" ng-show="success">
					<a href="#" class="close" data-dismiss="alert" ng-click="closeMsg()"
                ...
                <script type="text/ng-template" id="display">
                    ...
                    <td>
                        <button type="button" class="btn btn-primary btn-sm" ng-click="showEdit(data)">
                            Edit</button>
                        <button type="button" class="btn btn-danger btn-sm" ng-click="deleteData(data.id)">
                            Delete</button>
                    </td>
                </script>
                <!--Edit-->
                <script type="text/ng-template" id="edit">
                    <td><input type="text" ng-model="formData.first_name"
                            class="form-control" /></td>
                    <td><input type="text" ng-model="formData.last_name"
                            class="form-control" /></td>
                    <td>
                        <input type="hidden" ng-model="formData.data.id" />
                        <button type="button" class="btn btn-info btn-sm" ng-click="editData(data.id)">
                            Save</button>
                        <button type="button" class="btn btn-default btn-sm" ng-click="reset()">
                            Cancel</button>
                    </td>
                </script>
            </div>
...
<script>
    ...
    app.controller("liveController", function($scope, $http){
        ...
        $scope.deleteData = function(id){
            if(confirm("Are you sure you wany to remove it?")){
                $http({
                    method: "POST",
                    url: "delete.php",
                    data: {'id': id}
